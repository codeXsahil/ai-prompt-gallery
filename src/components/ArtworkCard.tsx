...
export const ArtworkCard = ({ artwork, onCopy, onShare, onClick }: ArtworkCardProps) => {
  return (
    <div
      className="group relative bg-card rounded-xl overflow-hidden border border-border hover:border-primary/50 transition-all duration-300 hover:shadow-2xl hover:shadow-primary/10 flex flex-col"
      itemScope
    ...
    >
      ...
      >
        <img
          src={optimizeImage(artwork.imageUrl)}
          alt={`AI Art: ${artwork.description || artwork.prompt}`}
          className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-500"
          itemProp="contentUrl"
          loading="lazy"
        />
        <meta itemProp="name" content={artwork.description || "AI Generated Artwork"} />
        <meta itemProp="author" content={artwork.uploaderName || "Prompt Vision Gallery"} />
        <meta itemProp="datePublished" content={artwork.createdAt?.seconds ? new Date(artwork.createdAt.seconds * 1000).toISOString() : new Date().toISOString()} />
      ...
      <div className="p-4 flex flex-col flex-grow">
        <h3 className="font-semibold text-foreground truncate mb-1" title={artwork.description} itemProp="caption">
        ...
        <div className="mt-auto bg-background rounded-lg p-3 border border-border group-hover:border-primary/30 transition-colors">
          <p className="text-muted-foreground text-xs line-clamp-3 font-mono mb-3 select-none" itemProp="description">
